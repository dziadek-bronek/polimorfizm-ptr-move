...
template <typename CHILD> struct CChildCreator : CParentCreator
{
    CChildCreator() = delete;
    CChildCreator(int number)
        : CParentCreator(number)
    {
...
int main()
{
    try
    {
        ...
        std::list<std::unique_ptr<CParentCreator>>
            mapOfEventsAndChildrenCreators;
