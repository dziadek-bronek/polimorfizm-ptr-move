...
template <typename T> struct CChildCreator : CParentCreator
{
    virtual std::unique_ptr<CParent> createNew()
    {
        return std::unique_ptr<CParent>(new T);
    }
};

std::vector<CParentCreator*> mapOfEventsAndChildren{0, 0, 0, 0, 0};

template <typename CHILD> void mapChildCreatorToEvent(int event)
{
...
std::unique_ptr<CParent> resolveEventToAChild(int event)
{
    ...
    for (int i = 1; i < mapOfEventsAndChildren.size(); ++i)
    {
        if (i == event)
        {
            ...
            {
                return std::unique_ptr<CParent>(
                    ((mapOfEventsAndChildren[i])->createNew()));
            }
        }
    }
...
int main()
{
    try
    {
        ...
        CInput input;
        input.init(std::unique_ptr<std::vector<int>>(
            new std::vector<int>{1, 3, 2, 2, 1, 2, 1, 1, 2, 5}));
